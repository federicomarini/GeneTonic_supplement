...
rld_macrophage <- rlogTransformation(dds_macrophage)
rld_macrophage <- rld_macrophage[, rld_macrophage$condition %in% c("naive", "IFNg")]


pheatmap::pheatmap(as.matrix(dist(t(assay(rld_macrophage)))))

pcaExplorer::pcaplot(rld_macrophage,
                     ntop = 5000,
...
results_IGNg_vs_naive <- results(dds_macrophage,
                                 name = "condition_IFNg_vs_naive",
                                 lfcThreshold = 1,
                                 alpha = FDR)
summary(results_IGNg_vs_naive)

results_IGNg_vs_naive$SYMBOL <- anno_df$gene_name[match(rownames(results_IGNg_vs_naive), anno_df$gene_id)]

ideal::plot_ma(results_IGNg_vs_naive, ylim = c(-5, 5), title = "MAplot - INFg vs Naive")

tbl_DEres_IGNg_vs_naive <- deseqresult2df(results_IGNg_vs_naive, FDR = FDR)

DT::datatable(tbl_DEres_IGNg_vs_naive, rownames = FALSE)
```
...
GObps_INFg_vs_naive <- topGOtable(
  DEgenes = tbl_DEres_IGNg_vs_naive$SYMBOL,
...
```{r clusterProfiler}
# clusterProfiler --------------------------------------------------------------
degenes_macrophage <- deseqresult2df(results_IGNg_vs_naive, FDR = FDR)$id
...
# g:profiler -------------------------------------------------------------------
degenes <- deseqresult2df(results_IGNg_vs_naive, FDR = FDR)$SYMBOL
